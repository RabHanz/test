...
const GameEngine = forwardRef<GameEngineRef, GameEngineProps>(({
  ...
  onMilestoneReached,
  onPauseStateChange,
  isContinuation = false, // Default to false
  initialScore = 0,
  initialCoinsThisRun = 0,
  initialDifficulty = 1,
  initialGameSpeed = BASE_GAME_SPEED,
  initialPerfectRun = true,
  ...
  useImperativeHandle(ref, () => ({
    resumeAfterAdContinue: () => {
      if (birdRef.current) {
        ...
        if (animationFrameIdRef.current === 0) {
          lastFrameTimeRef.current = performance.now(); // Reset lastFrameTime
    ...
    getGameState: () => ({
      ...
      gameSpeed: gameSpeedRef.current // or engineHud.currentSpeed
    }),
    triggerScreenshot: () => {
  ...
  const resetGame = useCallback((continuingRun = false) => {
    ...
    // For revive, hasRevivedRef should NOT be reset if continuing,
    // but should be reset for a truly new game.
    // App.tsx can pass an initialHasRevived prop if needed for multi-stage revives across sessions.
    // For a single revive per "run" (including continuations):
    if (!continuingRun) {
        hasRevivedRef.current = false;
    }
    // If continuing, hasRevivedRef retains its state from before the continue.
    ...
    isWaitingForContinueDecisionRef.current = false; // Always reset this for resetGame call


    // Reset game objects that should always be fresh (pipes, enemies on screen etc.)
    pipesRef.current = [];
    coinsRef.current = []; // Coins on screen are cleared. Collected coins are handled below.
    enemiesRef.current = [];
    ...
    // Reset or preserve run-specific state based on `continuingRun`
    if (!continuingRun) {
      // Truly new game
      scoreRef.current = 0;
      coinsCollectedThisRunRef.current = 0;
      powerGaugeRef.current = 0; // Reset power gauge for a new game
      gameSpeedRef.current = BASE_GAME_SPEED;
      perfectRunRef.current = true;
      allowFirstCrashRef.current = true; // Reset free crash for a new game
      timesContinuedThisRunRef.current = 0; // Reset continue counter for a new game
      lastMilestoneScoreAwardedRef.current = 0;
      enemiesDefeatedThisRunRef.current = 0;
      powerupsUsedThisRunRef.current = 0;

      const skillMultiplier = {
        'beginner': 0.8, 'intermediate': 1.0, 'advanced': 1.2, 'expert': 1.4
      ...
      // Continuing a run
      scoreRef.current = initialScore; // Use prop passed from App.tsx
      coinsCollectedThisRunRef.current = initialCoinsThisRun; // Use prop
      difficultyRef.current = initialDifficulty; // Use prop
      gameSpeedRef.current = initialGameSpeed; // Use prop
      perfectRunRef.current = initialPerfectRun; // Use prop

      // For continuation, 'resumeAfterAdContinue' in App.tsx typically gives a shield
      // So, currentPowerupRef and powerupTimeRef will be set there.
      // The powerGaugeRef could be preserved or reset based on design.
      // Let's assume powerGaugeRef is preserved on continue. If it was passed as a prop:
      // powerGaugeRef.current = initialPowerGauge || 0;
      // Or if it's meant to be reset on continue, set to 0 here. For now, let it persist.

      // allowFirstCrashRef is typically true after a continue (new chance)
      allowFirstCrashRef.current = true;
      // timesContinuedThisRunRef is managed by App.tsx and should not be reset here for a continuation.
      // lastMilestoneScoreAwardedRef, enemiesDefeatedThisRunRef, powerupsUsedThisRunRef should persist.
      // These would need to be passed as props if App.tsx manages them for continuation.
      // For simplicity if GameEngine is the source of truth for these during a run:
      // They are already in their refs and are not reset here if continuingRun is true.
    }

    // Common state resets for both new game and continuation
    comboRef.current = 0; // Combo typically resets after a "death" or pause leading to continue
    comboTimerRef.current = 0;
    activeDebuffRef.current = null; // Debuffs are cleared on continue/revive

    // Scene selection with enhanced logic (should run for both new and continued games to set up the visual)
    // If continuing, you might want to restore the exact scene index, but for now, it picks a new one
    // or continues transition logic if it was in progress.
    // If you want to persist the exact scene, that also needs to be passed as a prop.
    if (!continuingRun) { // Only pick a new random scene if it's a truly new game
        if (isZenMode) {
    ...
    } // If continuing, currentSceneIndexRef and nextSceneIndexRef should ideally be preserved or passed as props.
      // For now, this logic means a continued game might start in a new scene visually if resetGame is called.
      // A better approach for scene persistence on continue would be to pass `initialSceneIndex` as a prop.

    if (SCENES[currentSceneIndexRef.current]?.name === 'Night' && !isZenMode) initStars();


    // Reset flags and states common to both or specific to the start of play
    screenShakeRef.current = 0;
    pausedRef.current = false;
    onPauseStateChange(false);
    magnetFieldRef.current = false;

    gameFrameRef.current = 0; // Always reset frame count for physics calculations relative to start/resume
    lastFrameTimeRef.current = performance.now();

    // Reset analytics for a truly new game
    if (!continuingRun) {
        encounteredWeatherTypesThisRunRef.current.clear();
        const initialScene = SCENES[currentSceneIndexRef.current];
        if (initialScene && !isZenMode) { // Check if initialScene is defined
          encounteredWeatherTypesThisRunRef.current.add(initialScene.weather);
          onAchievementProgress('weatherWizard', 1);
        }
    }

    // Initialize player behavior tracking (always reset this)
    ...
    // Speed here should use the potentially continued gameSpeedRef.current
    const moveSpeed = (isZenMode ? 1.5 : gameSpeedRef.current) * (1 + difficultyRef.current * 0.1);
    const currentSceneForElements = SCENES[currentSceneIndexRef.current];
    if (currentSceneForElements) { // Ensure currentSceneForElements is defined
        initSceneSpecificElements(currentSceneForElements, moveSpeed);
    }


    // Start background music
    const sceneTypeForMusic = SCENES[currentSceneIndexRef.current]?.type;
    if (sceneTypeForMusic) { // Ensure sceneTypeForMusic is defined
    ...
    // Start game loop only if not paused and not waiting for continue decision
    // (isWaitingForContinueDecisionRef should be false here due to earlier reset)
    if (!pausedRef.current && !isWaitingForContinueDecisionRef.current) {
      lastFrameTimeRef.current = performance.now(); // Reset lastFrameTime just before starting loop
      animationFrameIdRef.current = requestAnimationFrame(gameLoop);
    }
    ...
    initialScore, initialCoinsThisRun, initialDifficulty, initialGameSpeed, initialPerfectRun
  ]);
...
// Main game loop with enhanced features
  const gameLoop = useCallback((timestamp: number) => {
    ...
    enemiesRef.current = enemiesRef.current.filter(enemy => {
      ...
      if (birdRef.current && !godModeRef.current && !isPhasingRef.current) {
        ...
        if (distance < collisionRadius) { // Collision occurred
          enemiesDefeatedThisRunRef.current++; // Count as engaged/defeated

          // --- Scenario 0: Bird has Ice Falcon's freeze ability ---
          // This should trigger on contact, regardless of other outcomes for THIS enemy.
          // freezeFieldActiveRef is a boolean on the bird (GameEngine state) indicating if the ability is ready.
          // It should be set to true when an Ice Falcon is selected.
          // Assuming freezeFieldActiveRef is set to true for Ice Falcon and reset after use or a timer.
          const currentSkyPeckerConfig = ENHANCED_SKYPECKER_TYPES[birdRef.current.typeIndex];
          if (currentSkyPeckerConfig.freezeEnemiesOnHit && freezeFieldActiveRef.current) {
            enemiesRef.current.forEach(otherEnemy => {
              if (otherEnemy !== enemy) { // Don't freeze the enemy being hit, just others nearby perhaps.
                                        // Or freeze all including the one hit if that's the design.
                                        // For now, let's assume it freezes OTHER enemies.
                otherEnemy.frozenDuration = 180; // 3 seconds
              }
            });
            // Potentially reset freezeFieldActiveRef.current here if it's a one-time effect per run/cooldown
            // For now, let's assume it's always active for Ice Falcon on hit
            // freezeFieldActiveRef.current = false; // If it's a one-shot per some condition
            addGameParticles(birdRef.current.x, birdRef.current.y, '#87CEEB', 20, 'game', 3, 60); // Ice particles
            Sounds.powerup(); // Or a specific freeze sound
          }


          // --- Scenario 1: Bird has a specific ability to negate THIS debuff ---
          if (canNegateDebuffRef.current && activeDebuffRef.current === null && enemy.debuffType) {
            canNegateDebuffRef.current = false;
            addGameParticles(birdRef.current.x, birdRef.current.y, ENHANCED_PARTICLE_COLORS.debuffNegated || '#90EE90', 12, 'game', 4, 50);
            Sounds.debuffNegated(); // Sound for debuff negation
            return false; // Remove enemy, no crash, no debuff applied
          }

          // --- If not negated by a specific bird ability, proceed ---
          let isHardCrash = true; // Assume it's a hard crash by default

          // Check if this enemy interaction should just apply a debuff without being a "hard crash"
          if (enemy.debuffType) {
            if (!activeDebuffRef.current) { // Bird is NOT currently debuffed: Apply debuff as a "soft hit"
              ...
              if (debuffConfig) {
                ...
                const soundFnName = `debuff${enemy.debuffType.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join('')}` as keyof typeof Sounds;
                if (typeof Sounds[soundFnName] === 'function') {
                    (Sounds[soundFnName] as () => void)();
                } else {
                    Sounds.pipeHit();
                }
              }
              perfectRunRef.current = false;
              addGameParticles(enemy.x, enemy.y, ENHANCED_PARTICLE_COLORS.debuffSprite || '#FFA500', 10, 'game');
              isHardCrash = false;
            }
          }

          // --- Process the outcome of the collision ---
          if (isHardCrash) {
            perfectRunRef.current = false;
          ...
          // If the game didn't end from handleFatalHitLogic(), remove the enemy.
          return false; // Remove the enemy in all collision scenarios (negated, soft debuff, or hard crash)
        }
      }
    ...
    if (nextScene && transitionProgress > 0) {
      ...
      const currentColorRGB = hexToRgb(currentScene.skyGradient.top);
      const nextColorRGB = hexToRgb(nextScene.skyGradient.top);

      const currentBottomColorRGB = hexToRgb(currentScene.skyGradient.bottom);
      const nextBottomColorRGB = hexToRgb(nextScene.skyGradient.bottom);

      if (currentColorRGB && nextColorRGB && currentBottomColorRGB && nextBottomColorRGB) {
        const blendedTopColorString = blendColors(currentColorRGB, nextColorRGB, transitionProgress);
        const blendedBottomColorString = blendColors(currentBottomColorRGB, nextBottomColorRGB, transitionProgress);

        const gradient = ctx.createLinearGradient(0, 0, 0, CANVAS_HEIGHT);
        gradient.addColorStop(0, blendedTopColorString);
        gradient.addColorStop(1, blendedBottomColorString);
        ctx.fillStyle = gradient;
      } else {
        // Fallback if colors are invalid (should be rare with valid constants)
        console.error("Error creating transitioning gradient due to invalid hex colors. Scene:", currentScene.name, "Next Scene:", nextScene?.name);
        const gradient = ctx.createLinearGradient(0, 0, 0, CANVAS_HEIGHT);
    ...
    selectedTrailEffect, playerSkillLevel, initSceneSpecificElements
  ]);
  ...
  }, [resetGame, handleKeyDown, handlePointerDown, handlePointerUp, selectedSkyPeckerTypeIndex, isContinuation]);
