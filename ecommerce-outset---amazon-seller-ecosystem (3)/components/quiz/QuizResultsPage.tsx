...
const PersonaDisplay: React.FC<{ persona: PersonaType, isPrimary: boolean }> = ({ persona, isPrimary }) => {
  const PersonaIcon = persona.Icon || LaunchIcon; // Fallback Icon
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left">
        <PersonaIcon className={`w-16 h-16 sm:w-24 sm:h-24 mb-4 sm:mb-0 sm:mr-6 flex-shrink-0 ${persona.accentColorClass}`} />
        <div>
          <h3 className={`text-2xl sm:text-3xl font-bold ${persona.accentColorClass} mb-2`}>
...
const QuizResultsPage: React.FC<QuizResultsPageProps> = ({ result, onRetake, onClose }) => {
  const { primaryPersona, secondaryPersonas, allScores } = result;

  if (!primaryPersona) {
    // Fallback for unknown or tied scores - could offer a generic path or retake
    ...
    return (
      <div className="p-4 md:p-8 text-center flex flex-col justify-center items-center h-full">
        <h2 className="text-2xl font-bold text-white mb-4">Interesting Profile!</h2>
        ...
        <Button variant="primary" onClick={() => { trackCTAClick('Results Contact Us'); window.location.href = '#contact'; onClose(); }} className="mb-4">
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-8 h-full overflow-y-auto">
      <div className="text-center">
        <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-2">Your Personalized Results!</h2>
      ...
      <div className="mt-8 text-center space-y-4 py-6 border-t border-gray-700">
        ...
        <Button variant="secondary" onClick={onRetake} className="w-full max-w-xs mx-auto">Retake Quiz</Button>
        <Button variant="link" onClick={onClose} className="text-gray-400 hover:text-orange-400 w-full max-w-xs mx-auto !mt-2">Close</Button>
      </div>
