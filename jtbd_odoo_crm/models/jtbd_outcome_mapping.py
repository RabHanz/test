...
class JtbdOutcomeMapping(models.Model):
    ...
    # Removed default lambda, will be handled by default_get
    ...
    job_statement = fields.Text(
        string='Job Statement', readonly=True, compute='_compute_job_statement', store=False, # Computed dynamically
    ...
    primary_outcome_progress = fields.Float(
        string='Progress (%)', compute='_compute_primary_outcome_progress', store=True, digits=(16, 1), # Use digits for precision
    ...
    def _compute_job_statement(self):
        """ Fetch the job statement from the related opportunity. """
        for record in self:
            # Ensure lead_id exists before accessing its fields
            if record.lead_id:
    ...
    def _compute_primary_outcome_progress(self):
        """ Calculate the progress percentage for the primary outcome. """
        for record in self:
            ...
            if isinstance(current_val, (int, float)) and isinstance(target_val, (int, float)):
                if target_val == current_val:
                    # If target is met exactly or already there
                    record.primary_outcome_progress = 100.0
                else:
                    # Calculate baseline (initial starting point for progress)
                    # In most cases, baseline is the current value, but if target is lower,
                    # we might invert the scale conceptually. For simplicity here,
                    # let's assume progress is always towards target from current.
                    baseline_value = record.current_value # Use current value as baseline

                    # Calculate total required change
                    total_change_needed = target_val - baseline_value
                    # Calculate change achieved so far
                    change_achieved = current_val - baseline_value # This is 0 if baseline is current

                    if total_change_needed != 0:
                        # Calculate progress percentage
                        progress = (change_achieved / total_change_needed) * 100
                        # Handle cases where progress might be negative (moved away) or > 100 (overshot)
                        # Often capped at 0-100 for display purposes. Let's cap it here.
                        # Note: This simple calc assumes higher is better. Needs adjustment
                        # if the goal is to *reduce* a value (e.g., reduce cost).
                        # A better approach might involve defining goal direction (increase/decrease).
                        # For now, capping:
                        record.primary_outcome_progress = max(0.0, min(100.0, progress if total_change_needed > 0 else (100-progress if total_change_needed<0 else 0) ))
                        # Simplified example assuming higher is better:
                        # progress = ((current_val - baseline_value) / (target_val - baseline_value)) * 100
                        # record.primary_outcome_progress = max(0.0, min(100.0, progress))

                    else: # Target equals baseline, progress is 100% if current also equals target
                        record.primary_outcome_progress = 100.0 if current_val == target_val else 0.0
            else:
                # If values are not valid numbers, progress is 0
    ...
    def action_apply_template(self):
        ...
        return {
            ...
            'context': {
                ...
                # Pass the category needed for the domain filter
                'default_mapping_job_category': self.job_category,
            }
        }

    def _prepare_suggestion_wizard_action(self, suggestion_type, title):
        """ Helper to open the suggestion wizard. """
        self.ensure_one()
        return {
            'name': title,
            'type': 'ir.actions.act_window',
            ...
            'context': {
                'default_mapping_id': self.id,
                'default_suggestion_type': suggestion_type,
            }
        }
    ...
    def action_update_opportunity(self):
        ...
        vals_to_write = {
            # Update opportunity fields with data from this mapping
            'jtbd_outcome_metric': self.outcome_metric,
            # Only update white_space if it's set on the mapping
        ...
        try:
            ...
            if self.white_space:
                body += _("<br/>- White Space Opportunities noted.")

            self.lead_id.message_post(body=body, subtype_id=self.env.ref('mail.mt_note').id)

            # Use the existing helper method for notifications
    ...
    def action_view_opportunity(self):
        """ Action to view the related opportunity. """
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('Opportunity'),
            ...
            'view_mode': 'form',
            'target': 'current', # Open in main window
        }

    def action_suggest_additional_outcomes(self):
        """Suggest additional outcomes based on job category."""
        return self._prepare_suggestion_wizard_action(
            'outcomes', _('Suggested Additional Outcomes')
        )

    def action_suggest_milestones(self):
        """Suggest milestones based on job category."""
        return self._prepare_suggestion_wizard_action(
            'milestones', _('Suggested Milestones')
        )

    def action_suggest_white_space(self):
        """Suggest white space opportunities."""
        return self._prepare_suggestion_wizard_action(
            'white_space', _('Suggested White Space Opportunities')
        )