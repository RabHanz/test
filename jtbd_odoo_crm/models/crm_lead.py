...
class CrmLead(models.Model):
    ...
    def _check_job_statement_structure(self):
        # Basic structural check, not foolproof but enforces presence
        for lead in self.filtered('jtbd_job_statement'): # Only check if statement exists
            statement = lead.jtbd_job_statement.lower()
            if not ("when " in statement and " i want to " in statement and " so i can " in statement):
                 raise ValidationError(_("Job Statement must follow the 'When..., I want to..., so I can...' structure."))
