...
class CrmLead(models.Model):
    ...
    jtbd_renewal_risk = fields.Integer(
        ...
        readonly=True, # Populated by prediction/analysis later
        tracking=True,
        group_operator='avg',
        copy=False,
    ...
    jtbd_growth_velocity = fields.Float(
        string="Est. Growth Velocity (%)",
        tracking=True,
        group_operator='avg',
        digits=(16, 1), # Example precision
        help="Estimated annual growth rate or velocity metric for the prospect/client."
    )
    jtbd_competitive_position = fields.Selection(
        ...
        ('unknown', 'Unknown')],
        string="Competitive Position",
        tracking=True,
        help="Assessment of the prospect's position within their market."
    )
