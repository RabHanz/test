...
class JtbdOutcomeSuggestionWizard(models.TransientModel):
    ...
    # No default_get override needed - create handles population
    @api.model_create_multi
    def create(self, vals_list):
        # Create the main wizard record(s) first
        wizards = super().create(vals_list)
        suggested_item_obj = self.env['jtbd.suggested.item']
        for wizard in wizards:
            # Populate suggested items *after* wizard record is created
            if wizard.mapping_id and wizard.suggestion_type:
                try:
                    ...
                    for s_dict in suggestions_list_of_dicts:
                        # Final validation check before creating item
                        if isinstance(s_dict, dict) and s_dict.get('name') and isinstance(s_dict['name'], str) and s_dict['name'].strip():
                            ...
                            items_to_create.append(s_dict)
                             ...
                             _logger.warning(f"Skipping creation of invalid suggestion for wizard {wizard.id}: {s_dict}")

                    if items_to_create:
                        suggested_item_obj.create(items_to_create)
                except Exception as e:
                    _logger.error(f"Error creating suggested items for wizard {wizard.id}: {e}", exc_info=True)
            else:
                _logger.warning(f"Wizard {wizard.id} missing mapping_id or suggestion_type on create, cannot populate suggestions.")
        return wizards

    # _get_suggestions provides the list of dictionaries
    def _get_suggestions(self, mapping, suggestion_type):
        suggestions = [] # Holds dicts: {'name': '...', 'description': '...'}
        if not mapping or not mapping.job_category:
            return suggestions

        job_category = mapping.job_category
        patterns = self.env['jtbd.outcome.pattern'].search([
            ('job_category', '=', job_category), ('active', '=', True)
        ], limit=1)

        if not patterns: return suggestions
        pattern = patterns[0]

        # Helper ensures name is valid before adding dict to list
        def _add_suggestion(name_val, desc_val):
            name_str = str(name_val or '').strip()
            if name_str:
                 suggestions.append({'name': name_str, 'description': str(desc_val or '').strip()})
            ...
            if self.suggestion_type == 'outcomes':
                ...
                for i, item in enumerate(selected_items):
                     ...
                     if item_name.strip() and item_name not in existing_metrics:
                        ...
                        vals = { 'name': name_display, 'metric': metric, 'is_percentage': is_percentage,
                                'metric_unit': metric_unit, 'priority': 'medium',
