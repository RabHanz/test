...
class JtbdJobStatementBuilder(models.TransientModel):
    ...
    @api.model
    def _get_applicable_patterns_domain(self, category_id, lead_record):
        """ Helper to build domain for finding applicable patterns based on category and lead source. """
        if not category_id:
            return [('id', '=', 0)] # Return domain for no results

        domain = [('job_category_id', '=', category_id), ('active', '=', True)]

        # --- NEW: Filter by Source Applicability ---
        lead_source_label = lead_record.jtbd_data_source_label if lead_record else None
        _logger.debug(f"Wiz JB Build: Filtering patterns for category {category_id}, lead source '{lead_source_label}'")

        if lead_source_label:
            # If lead source is known, find patterns applicable to 'all' OR the specific source label
            # Assumes source labels match selection keys on the pattern.
            # Add logic here if mapping is needed (e.g., 'funnel_1' on lead maps to 'funnel_only' on pattern)
            applicable_sources = ['all', lead_source_label]
            # Example confidence mapping (adjust threshold as needed)
            lead_confidence = lead_record.jtbd_source_confidence_score or 0.0
            if lead_confidence >= 0.8:
                applicable_sources.append('high_confidence')

            domain.append(('jtbd_source_type_applicability', 'in', applicable_sources))
            _logger.debug(f"Wiz JB Build: Adding source applicability domain: {domain[-1]}")
        else:
            # If lead source is unknown, maybe only show 'all' or broaden filter?
            # For now, let's assume we show 'all' applicable patterns if source is unknown
            domain.append(('jtbd_source_type_applicability', '=', 'all'))
            _logger.debug(f"Wiz JB Build: Lead source unknown, filtering for applicability 'all'.")
        # --- End Source Applicability Filter ---

        # Add other filters later (agency type, tech stack) if lead fields exist for them
    ...
    def _default_get_suggested_patterns(self, category_id, lead):
        """ Get initial suggested patterns using the domain helper. """
        if not category_id or not lead:
            return [(5, 0, 0)]
        domain = self._get_applicable_patterns_domain(category_id, lead)
        patterns = self.env['jtbd.job.pattern'].search(domain, limit=5, order='sequence asc')
        _logger.debug(f"Wiz JB Build (Default): Found {len(patterns)} patterns for domain {domain}")
    ...
    def _find_suggested_patterns(self):
        """ Find suggested patterns using the domain helper (called on state change). """
        self.ensure_one()
        if not self.job_category_id or not self.lead_id:
            self.suggested_patterns = [(5, 0, 0)]
            return
        domain = self._get_applicable_patterns_domain(self.job_category_id.id, self.lead_id)
        patterns = self.env['jtbd.job.pattern'].search(domain, limit=5, order='sequence asc')
        _logger.debug(f"Wiz JB Build (Find): Found {len(patterns)} patterns for domain {domain}")
        self.suggested_patterns = [(6, 0, patterns.ids)] if patterns else [(5, 0, 0)]
