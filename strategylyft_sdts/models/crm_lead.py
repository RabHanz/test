...
class CrmLead(models.Model):
    ...
    jtbd_attribution_summary = fields.Text(
        ...
        copy=False,
        tracking=True, # Track changes pushed from external system
        help="Stores key results or a summary (potentially JSON formatted text) from the external ML-driven Cross-Approach Attribution model, reflecting channel/touchpoint influence across the unified journey."
    )
    ...
    # --- Add Fields for PRD3 Enhanced Flow ---

    # Data Provenance / Source / Confidence
    jtbd_data_source_label = fields.Selection(
        selection=lambda self: self.env['jtbd.data.provenance']._fields['data_source_label'].selection,
        string='Primary Data Source', index=True, tracking=True,
        help="The primary source label for the initial data associated with this lead (e.g., Funnel, Scraped, Manual)."
    )
    jtbd_source_confidence_score = fields.Float(
        string='Source Confidence Score', digits=(3, 2), tracking=True, aggregator='avg',
        help="Confidence score (0.0 to 1.0) reflecting the reliability of the primary data source."
    )
    # Maybe add a link to the specific provenance record? Optional for now.
    # jtbd_data_provenance_id = fields.Many2one('jtbd.data.provenance', ...)

    # Contact Progression Pipeline (CPP) Status
    jtbd_contact_progression_status = fields.Selection(
        [('outbound_prospect', 'Outbound Prospect'), # Identified, not yet engaged
         ('outbound_engaging', 'Outbound Engaging'), # Sequence started
         ('inbound_lead', 'Inbound Lead'), # Submitted form, quiz, etc.
         ('inbound_mql', 'Inbound MQL'), # Marketing Qualified
         ('sales_working', 'Sales Working (SQL/Opportunity)'),
         ('customer', 'Customer'),
         ('dormant', 'Dormant'),
         ('disqualified', 'Disqualified'),
         ('unknown', 'Unknown / Not Set')],
        string='Contact Progression Status', index=True, tracking=True, default='unknown',
        help="Current stage in the unified contact/prospect pipeline."
    )

    # --- Outbound Intelligence / Execution Fields (REVISED) ---
    jtbd_is_outbound_targeted = fields.Boolean( string="Targeted for Outbound?", default=False, index=True, tracking=True)
    jtbd_outreach_priority_score = fields.Integer( string="Outreach Priority Score", readonly=True, copy=False, tracking=True, aggregator='avg')
    jtbd_suggested_sequence_id = fields.Char( # Keeping Char, external system provides ID
        string="Suggested Outbound Sequence ID", readonly=True, copy=False, tracking=True,
        help="Identifier for the suggested outbound engagement sequence template."
    )
    # jtbd_outbound_engagement_summary = fields.Text(...) # REMOVED Placeholder Text Field

    # NEW O2M to detailed engagement logs
    jtbd_outbound_engagement_ids = fields.One2many(
        'jtbd.outbound.engagement', 'lead_id', string='Outbound Engagements', readonly=True
    )
    # --- End Outbound Fields ---

    # --- Add Field for Source Interaction Tagging (SIT) ---
    jtbd_source_interaction_tags = fields.Many2many(
        'crm.tag', # Use standard CRM Tags
        'jtbd_crm_lead_source_interaction_tag_rel', # Relation table
        'lead_id', 'tag_id', # Column names
        string="Source Interaction Tags",
        tracking=True,
        help="Tags describing the nature of the interaction based on the data source (e.g., 'Webinar Attended', 'Pricing Page Visit', 'Scraped Keyword Match')."
    )
    # --- End SIT Field ---

    # --- Unified Journey Tracking (PRD3 V4) ---
    jtbd_unified_journey_event_ids = fields.One2many(
        'jtbd.unified.journey.event', 'lead_id', string='Unified Journey Events',
        readonly=True, # Usually read-only on lead form, populated by automation
        help="Log of all significant interactions across inbound, outbound, and CRM activities."
    )
    # --- End Unified Journey Tracking ---
...
#                _logger.error(f"Error counting related records for lead {lead.id}: {e}", exc_info=True)
    ...
    def write(self, vals):
        # Track stage changes BEFORE writing
        stage_change_events = []
        if 'stage_id' in vals:
            new_stage_id = vals['stage_id']
            stage_model = self.env['crm.stage']
            for lead in self.filtered(lambda l: l.stage_id.id != new_stage_id): # Only log if stage actually changes
                try:
                    old_stage_name = lead.stage_id.name or 'N/A'
                    new_stage_name = stage_model.browse(new_stage_id).name or 'N/A'
                    stage_change_events.append({
                        'lead_id': lead.id,
                        'event_datetime': fields.Datetime.now(),
                        'event_type': 'stage_change',
                        'source_system': 'odoo_crm',
                        'channel': 'CRM UI/Automation',
                        'description': f"Stage changed from '{old_stage_name}' to '{new_stage_name}'",
                        'related_record_ref': f'{lead._name},{lead.id}',
                    })
                except Exception as e:
                     _logger.warning(f"Failed to prepare stage change event for lead {lead.id}: {e}")

        # Also track progression status changes
        if 'jtbd_contact_progression_status' in vals:
            new_status = vals['jtbd_contact_progression_status']
            status_dict = dict(self._fields['jtbd_contact_progression_status'].selection)
            for lead in self.filtered(lambda l: l.jtbd_contact_progression_status != new_status):
                 try:
                     old_status_name = status_dict.get(lead.jtbd_contact_progression_status, 'N/A')
                     new_status_name = status_dict.get(new_status, 'N/A')
                     stage_change_events.append({ # Reuse the list for efficiency
                         'lead_id': lead.id,
                         'event_datetime': fields.Datetime.now(),
                         'event_type': 'status_change',
                         'source_system': 'odoo_jtbd', # Likely changed by our module/automation
                         'channel': 'CRM UI/Automation',
                         'description': f"Progression Status changed from '{old_status_name}' to '{new_status_name}'",
                         'related_record_ref': f'{lead._name},{lead.id}',
                     })
                 except Exception as e:
                      _logger.warning(f"Failed to prepare status change event for lead {lead.id}: {e}")


        # Call original write method
        result = super(CrmLead, self).write(vals)

        # Create journey events AFTER successful write
        if stage_change_events:
            try:
                self.env['jtbd.unified.journey.event'].create(stage_change_events)
            except Exception as e:
                 _logger.error(f"Failed to create journey event(s) after crm.lead write: {e}", exc_info=True)
